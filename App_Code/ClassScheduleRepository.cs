...
public class ClassScheduleRepository : System.Web.UI.Page
{
    ...
    public static ClassTemplate _0050010001_View_Case_ID(string Case_ID)
    {
        string sqlstr = @"SELECT * FROM ProjectData WHERE Case_ID=@Case_ID";      // LaborTemplate > CaseData              002
        var list = DBTool.Query<ClassTemplate>(sqlstr, new { Case_ID = Case_ID });
        if (list.Count() > 0)
            return list.First();
        else
    ...
    public static ClassTemplate _0050010002_View_Case_ID(string Case_ID)
    {
        string sqlstr = @"SELECT * FROM Business_Process_Data WHERE Case_ID=@Case_ID";      // LaborTemplate > CaseData              002
        var list = DBTool.Query<ClassTemplate>(sqlstr, new { Case_ID = Case_ID });
        if (list.Count() > 0)
            return list.First();
        else
    ...
    public static ClassTemplate _0050010010_View_Case_ID(string Case_ID)
    {
        string sqlstr = @"SELECT * FROM AccountingData WHERE Case_ID=@Case_ID";      // LaborTemplate > CaseData              002
        var list = DBTool.Query<ClassTemplate>(sqlstr, new { Case_ID = Case_ID });
        if (list.Count() > 0)
            return list.First();
        else
    ...
    public static ClassTemplate _0050010011_View_Case_ID(string Case_ID)
    {
        string sqlstr = @"SELECT * FROM AccountingData WHERE Case_ID=@Case_ID";      // LaborTemplate > CaseData              002
        var list = DBTool.Query<ClassTemplate>(sqlstr, new { Case_ID = Case_ID });
        if (list.Count() > 0)
